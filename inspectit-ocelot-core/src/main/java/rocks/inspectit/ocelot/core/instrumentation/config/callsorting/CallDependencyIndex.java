...
public class CallDependencyIndex {
    ...
    public void add(CallDependencies actionCall) {
        actionCall.getWrites().forEach(key -> writesIndex.computeIfAbsent(key, (k) -> new HashSet<>()).add(actionCall));
        actionCall.getReadsBeforeWritten()
                .forEach(key -> readsBeforeWrittenIndex.computeIfAbsent(key, (k) -> new HashSet<>()).add(actionCall));
    }
