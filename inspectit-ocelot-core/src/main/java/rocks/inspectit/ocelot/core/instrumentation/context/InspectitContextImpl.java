...
public class InspectitContextImpl implements InternalInspectitContext {
    ...
    public void enterSpan(Span span) {
        if (currentSpanScope == null) {
            try {
                currentSpanScope = Instances.logTraceCorrelator.startCorrelatedSpanScope(() -> Tracing.getTracer()
                        .withSpan(span));
            } catch (Throwable t) {
    ...
    public Scope enterFullTagScope() {
        ...
        dataTagsStream().forEach(e -> builder.putLocal(TagKey.create(e.getKey()), TagUtils.createTagValue(e.getValue()
                .toString())));
    ...
    private Iterator<Tag> getPostEntryPhaseTags() {
        return postEntryPhaseDownPropagatedData.entrySet()
                ...
                .filter(e -> ALLOWED_TAG_TYPES.contains(e.getValue().getClass()))
                .map(e -> Tag.create(TagKey.create(e.getKey()), TagUtils.createTagValue(e.getValue()
