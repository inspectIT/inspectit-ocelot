...
@Getter
@Slf4j
public class PersistedAttributesReaderWriter {
    ...
    private PersistedAttributesReaderWriter(final ObjectMapper mapper, @NotNull final Path path) {
        this.mapper = mapper;
        this.path = path;
    }

    public static PersistedAttributesReaderWriter of(final String filenameInput) {
        final Path path = Paths.get(filenameInput);
        return new PersistedAttributesReaderWriter(new ObjectMapper(), path);
    }
