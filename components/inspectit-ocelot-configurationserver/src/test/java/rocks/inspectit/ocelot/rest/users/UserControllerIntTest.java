...
public class UserControllerIntTest extends IntegrationTestBase {
    ...
    class SelectUsers {
        ...
        void noUsernameFilter() {
            ...
            ResponseEntity<List<User>> result = authRest.exchange("/api/v1/users", HttpMethod.GET, null, new ParameterizedTypeReference<List<User>>() {
            });
            assertThat(result.getStatusCode()).isEqualTo(HttpStatus.OK);
            assertThat(result.getBody()).hasSize(2).anySatisfy((u) -> {
        ...
        void emptyUsernameFilter() {
            ...
            ResponseEntity<List<User>> result = authRest.exchange("/api/v1/users?username=", HttpMethod.GET, null, new ParameterizedTypeReference<List<User>>() {
            });
            assertThat(result.getStatusCode()).isEqualTo(HttpStatus.OK);
            assertThat(result.getBody()).hasSize(2).anySatisfy((u) -> {
                assertThat(u.getUsername()).isEqualTo(settings.getDefaultUser().getName());
        ...
        void withUsernameFilter() {
            ...
            ResponseEntity<List<User>> result = authRest.exchange("/api/v1/users?username=JOhN", HttpMethod.GET, null, new ParameterizedTypeReference<List<User>>() {
            });
            assertThat(result.getStatusCode()).isEqualTo(HttpStatus.OK);
            assertThat(result.getBody()).hasSize(1).anySatisfy((u) -> {
                assertThat(u.getUsername()).isEqualTo("john");
                assertThat(u.getPassword()).isNull();
                assertThat(u.getPasswordHash()).isNull();
                assertThat(u.getId()).isNotNull();
    ...
    class GetUser {
        ...
        @Test
        void existingUser() {
            Long id = userService.addOrUpdateUser(User.builder().username("John").password("doe").build()).getId();

            ResponseEntity<User> result = authRest.exchange("/api/v1/users/" + id, HttpMethod.GET, null, new ParameterizedTypeReference<User>() {
            });
        ...
        void nonExistingUser() {
            ResponseEntity<User> result = authRest.exchange("/api/v1/users/123456789", HttpMethod.GET, null, new ParameterizedTypeReference<User>() {
            });
            User user = result.getBody();
    ...
    class AddUser {
        ...
        void usernameAlreadyExists() {
            ...
            ResponseEntity<String> result = authRest.postForEntity("/api/v1/users", userToAdd, String.class);
        ...
        void ensureIdIgnored() {
            ...
            ResponseEntity<User> result = authRest.postForEntity("/api/v1/users", userToAdd, User.class);
            User user = result.getBody();
            ...
            ResponseEntity<List<User>> authResult = rest.withBasicAuth("joHn", "DoE")
                    .exchange("/api/v1/users", HttpMethod.GET, null, new ParameterizedTypeReference<List<User>>() {
                    });
    ...
    class DeleteUser {
        ...
        void userNotExisting() {
            ResponseEntity<String> result = authRest.exchange("/api/v1/users/" + 123456789, HttpMethod.DELETE, null, String.class);
            assertThat(result.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
        }
        ...
        void userExisting() {
            ...
            ResponseEntity<String> result = authRest.exchange("/api/v1/users/" + id, HttpMethod.DELETE, null, String.class);
            assertThat(result.getStatusCode()).isEqualTo(HttpStatus.OK);
