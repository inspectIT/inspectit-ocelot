...
public class AgentConfigurationReloadTaskTest {
    ...
    @Nested
    class supplyAgentDocumentation {

        private static final String srcYaml = """
            inspectit:
              instrumentation:
                scopes:
        ...
        void verifySuccessfulSupply() {
            ...
            when(workspaceAccessor.configurationFileIsDirectory(anyString())).thenReturn(true);
            when(workspaceAccessor.listConfigurationFiles(anyString())).thenReturn(Collections.singletonList(fileInfo));
            when(workspaceAccessor.readConfigurationFile(anyString())).thenReturn(Optional.of(srcYaml));
            ...
            AgentConfigurationReloadTask task = new AgentConfigurationReloadTask(serializer, fileManager, consumer);
            AgentDocumentation documentation = new AgentDocumentation(fileName, Set.of("s_jdbc_statement_execute"));
            ...
            AgentConfiguration config = configurationList.stream().findFirst().get();
            assertThat(config.getDocumentations()).hasSize(1);
            assertThat(config.getDocumentationsAsMap()).hasSize(1);
            assertThat(config.getDocumentations()).contains(documentation);
        }

        @Test
        void verifyEmptySupply() {
            FileInfo fileInfo = mock(FileInfo.class);
            when(fileInfo.getAbsoluteFilePaths(any())).thenReturn(Stream.of(fileName), Stream.of(fileName));
            when(workspaceAccessor.agentMappingsExist()).thenReturn(true);
            when(workspaceAccessor.configurationFileExists(anyString())).thenReturn(true);
            ...
            assertThat(config.getDocumentations()).hasSize(1);
            assertThat(config.getDocumentationsAsMap()).hasSize(1);
            assertThat(config.getDocumentations()).contains(new AgentDocumentation(fileName, emptySet()));
        }
    }
}
