...
public class ConfigurationSchemaProvider {
    ...
    private static final Set<Class<?>> INTEGER_CLASSES = new HashSet<>(Arrays.asList(Byte.class, byte.class, Short.class, short.class, Integer.class, int.class, Long.class, long.class));

    private static final Set<Class<?>> FLOAT_CLASSES = new HashSet<>(Arrays.asList(Float.class, float.class, Double.class, double.class));

    private static final Set<Class<?>> BOOLEAN_CLASSES = new HashSet<>(Arrays.asList(Boolean.class, boolean.class));
    ...
    Optional<ConfigurationPropertyDescription> toDescription(PropertyDescriptor property) {
        if (property.getWriteMethod() != null && !isExcluded(property)) {
            ...
            ConfigurationPropertyDescription.ConfigurationPropertyDescriptionBuilder builder = ConfigurationPropertyDescription
                    .builder()
                    .propertyName(name)
            ...
            if (PropertyPathHelper.isBean(type)) {
                return Optional.of(builder.type(ConfigurationPropertyType.COMPOSITE)
    ...
    private String getReadableName(PropertyDescriptor property) {
        return getPropertyAnnotation(property, UISettings.class).map(UISettings::name)
                .orElse(camelCaseToReadableName(property.getName()));
    }

    @VisibleForTesting
    ConfigurationPropertyDescription.ConfigurationPropertyDescriptionBuilder setTerminalType(Type type, ConfigurationPropertyDescription.ConfigurationPropertyDescriptionBuilder builder) {
        if (type instanceof Class<?>) {
            Class<?> clazz = (Class<?>) type;
            if (clazz.isEnum()) {
                return builder.type(ConfigurationPropertyType.ENUM)
                        .enumValues(Arrays.stream(clazz.getEnumConstants())
                                .map(Object::toString)
    ...
    private boolean isNullable(PropertyDescriptor property) {
        ...
        return !getPropertyAnnotation(property, NotNull.class).isPresent() && !getPropertyAnnotation(property, NotBlank.class)
                .isPresent() && !getPropertyAnnotation(property, NotEmpty.class).isPresent();
    }
