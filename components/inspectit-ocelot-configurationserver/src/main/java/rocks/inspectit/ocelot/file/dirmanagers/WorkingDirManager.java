...
public class WorkingDirManager {
        ...
        try (Stream<Path> files = Files.list(dir)) {
            ...
            for (Path child : files.collect(Collectors.toList())) {
                if (Files.isDirectory(child)) {
                    result.addAll(listFiles(getRelativePath(child)));
                } else {
    ...
    private Path setDir(String path) throws AccessDeniedException {
        if (StringUtils.isEmpty(path)) {
            return filesRoot;
        } else {
            assertPathWithinFilesRoot(path);
            return filesRoot.resolve(path);
        }
    }
