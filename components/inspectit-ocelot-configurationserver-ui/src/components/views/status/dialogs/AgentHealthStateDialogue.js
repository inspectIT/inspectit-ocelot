...
const AgentHealthStateDialogue = ({ visible, onHide, error, contentValue, contextName }) => {
  ...
  if (history && history.length > 0) {
    ...
    history.forEach((value) => {
      ...
      historyElements.push(
        <div>
          <b>{time}</b> <span style={{ color: iconColor, fontWeight: 'bold' }}>{healthInfo}</span>{' '}
          <span style={{ fontFamily: 'monospace' }}>{source}:</span> <br /> <span style={{ fontStyle: 'italic' }}>{message}</span>
          <div
            style={{
              borderBottom: '0.05rem solid rgba(228, 228, 228, 0.8)',
              width: '95%',
              margin: 'auto',
              padding: '0.5rem',
