...
To build and export it run:
```bash
yarn export
```
The exported page will be in the `out` directory.
...
The Redux `Provider` is implemented in `pages/_app.js`. Since the `OcelotConfigurationUI` component is wrapped in `withReduxStore` the redux store will be automatically initialized and provided to `OcelotConfigurationUI`, which in turn passes it off to `react-redux`'s `Provider` component.
