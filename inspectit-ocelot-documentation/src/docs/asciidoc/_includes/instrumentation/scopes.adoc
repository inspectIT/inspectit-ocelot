...
2. Check if the superclass matcher matches the superclass of class `X` (if defined)
...
| false
| If this value is `true`, only methods will be instrumented which are inherited of a superclass or interface.
