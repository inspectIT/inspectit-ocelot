...
If any start-span-condition is defined, a span will only be created when all conditions are met.
Analogous to this, attributes will only be written if each condition defined in attribute-conditions is fulfilled.
The conditions that can be defined are equal to the ones of data providers, thus, please see the <<Adding Conditions, data provider conditions description>> for detailed information.

When any conditions under start-span-conditions are defined, a span is only created if all conditions are met.
Similarly to attach attributes all conditions under attribute-conditions have to be fulfilled. The conditions that can be used are exactly the same as the ones available for .