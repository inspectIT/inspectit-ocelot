...
They are defined by (A) specifying their input parameters and (B) giving a Java code snippet which defines 
...
which has the type `long`. Note that for input parameters automatic primitive unboxing is supported.
...
For special input parameters inspectIT automatically assigns the desired value.
