---
id: version-2.6.11-installation
title: Installation
...
```bash
$ java -jar inspectit-ocelot-agent-2.6.11.jar <PID> [<AGENT_ARGUMENTS>]
```
...
```bash
$ java -jar inspectit-ocelot-agent-2.6.11.jar 1337 '{"inspectit":{"service-name":"my-agent"}}'
```
...
```bash
$ ./jattach.sh <PID> load instrument false /path/to/inspectit-ocelot-agent-2.6.11.jar='{"inspectit.service-name" : "MyService"}'
```
