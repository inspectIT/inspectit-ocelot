...
```bash
$ java -jar inspectit-ocelot-agent-2.1.1.jar <PID> [<AGENT_ARGUMENTS>]
```
...
```bash
$ java -jar inspectit-ocelot-agent-2.1.1.jar 1337 '{"inspectit":{"service-name":"my-agent"}}'
```
...
```bash
$ ./jattach.sh <PID> load instrument false /path/to/inspectit-ocelot-agent-2.1.1.jar='{"inspectit.service-name" : "MyService"}'
```
