---
id: version-1.15.2-installation
title: Installation
...
```bash
$ java -javaagent:"/path/to/inspectit-ocelot-agent-1.15.2.jar" -jar my-java-program.jar
```
...
```bash
$ java -jar inspectit-ocelot-agent-1.15.2.jar <PID> [<AGENT_ARGUMENTS>]
```
...
```bash
$ java -jar inspectit-ocelot-agent-1.15.2.jar 1337 '{inspectit:{service-name:"my-agent"}}'
```
...
```bash
$ ./jattach.sh <PID> load instrument false /path/to/inspectit-ocelot-agent-1.15.2.jar='{"inspectit.service-name" : "MyService"}'
```
