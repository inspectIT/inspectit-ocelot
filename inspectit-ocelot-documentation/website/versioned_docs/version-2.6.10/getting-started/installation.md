---
id: version-2.6.10-installation
title: Installation
...
```bash
$ java -jar inspectit-ocelot-agent-2.6.10.jar <PID> [<AGENT_ARGUMENTS>]
```
...
```bash
$ java -jar inspectit-ocelot-agent-2.6.10.jar 1337 '{"inspectit":{"service-name":"my-agent"}}'
```
...
```bash
$ ./jattach.sh <PID> load instrument false /path/to/inspectit-ocelot-agent-2.6.10.jar='{"inspectit.service-name" : "MyService"}'
```
