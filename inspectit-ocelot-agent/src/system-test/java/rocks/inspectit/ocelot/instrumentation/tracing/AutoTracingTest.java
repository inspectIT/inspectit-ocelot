...
public class AutoTracingTest extends TraceTestBase {
    ...
    static void waitForInstrumentation() {
        TestUtils.waitForClassInstrumentation(AutoTracingTest.class, 15, TimeUnit.SECONDS);
    }
    ...
    void verifyStackTraceSampling() {
        ...
        assertTraceExported((spans) -> {
            ...
            SpanData nestedWait = getSpanWithName(spans, "*AutoTracingTest.nestedWait");
            SpanData passiveWait = getSpanWithName(spans, "*AutoTracingTest.passiveWait");


            assertThat(activeWait.getParentSpanId()).isEqualTo(root.getContext().getSpanId());
            assertThat(nestedWait.getParentSpanId()).isEqualTo(root.getContext().getSpanId());
            assertThat(passiveWait.getParentSpanId()).isEqualTo(nestedWait.getContext().getSpanId());

            assertThat(spans).anySatisfy((span) -> {
                assertThat(span.getName()).isEqualTo("*Thread.sleep");
                assertThat(span.getParentSpanId()).isEqualTo(passiveWait.getContext().getSpanId());
            });

            assertThat(activeWait.getEndTimestamp()).isLessThan(nestedWait.getStartTimestamp());
        });
    }
