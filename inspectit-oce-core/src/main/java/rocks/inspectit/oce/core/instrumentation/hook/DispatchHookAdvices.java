...
public class DispatchHookAdvices {
    ...
    public static <T> DynamicType.Builder<T> adviceOn(DynamicType.Builder<T> builder, ElementMatcher<? super MethodDescription> methods) {
        builder = builder.visit(Advice.to(NonStaticMethodAdvice.class).on(
                not(isStatic()).and(not(isConstructor())).and(methods)));
    ...
    private static class NonStaticMethodAdvice {
        ...
        public static void onEnter(@Advice.Origin Class<?> declaringClass, @Advice.Origin("#m#s") String signature,
                                   @Advice.AllArguments Object[] args, @Advice.This Object thiz,
                                   @Advice.Local("hook") IMethodHook hook, @Advice.Local("context") IInspectitContext context) {
            hook = Instances.hookManager.getHook(declaringClass, signature);
            context = hook.onEnter(args, thiz);
        }

        @Advice.OnMethodExit(onThrowable = Throwable.class)
        public static void onExit(@Advice.AllArguments Object[] args, @Advice.This Object thiz,
                                  @Advice.Return(typing = Assigner.Typing.DYNAMIC) Object returnValue, @Advice.Thrown Throwable thrown,
                                  @Advice.Local("hook") IMethodHook hook, @Advice.Local("context") IInspectitContext context) {
